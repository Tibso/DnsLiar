...
pub fn disable(con: &mut Connection, filter: &str, pattern: &str) -> RedisResult<ExitCode> {
    let keys: Vec<String> = con
        .scan_match(format!("DBL;D;{filter};{pattern}"))?
        .collect();
    if keys.is_empty() {
...
pub fn enable(con: &mut Connection, filter: &str, pattern: &str) -> RedisResult<ExitCode> {
    let keys: Vec<String> = con
        .scan_match(format!("DBL;D;{filter};{pattern}"))?
        .collect();
    if keys.is_empty() {
...
pub fn add_ips(
    ...
    let mut pipe = pipe();
    let keys: Vec<String> = ips
        .iter()
        .map(|ip| format!("DBL;I;{filter};{ip}"))
        .collect();
    for key in keys {
        pipe.hset_multiple(
            &key,
            &[("enabled", "1"), ("date", &get_date()), ("src", src)],
        )
...
/// Remove IP rules
pub fn remove_ips(con: &mut Connection, filter: &str, ips: Vec<String>) -> RedisResult<ExitCode> {
    if ips.iter().any(|ip| ip.parse::<IpAddr>().is_err()) {
        println!("ERR: An IP is invalid");
        return Ok(ExitCode::from(65));
    }
...
pub fn search(con: &mut Connection, filter: &str, pattern: &str) -> RedisResult<ExitCode> {
